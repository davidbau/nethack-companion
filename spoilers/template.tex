...
\texttt{\#tip} & tip a container out onto the floor\\
\texttt{\#force} & force a lock\\
\texttt{\#untrap} & disarm a trap (or trapped chest)\\
\texttt{\#rub} & rub a magic lamp or a touchstone\\
\texttt{\#enhance} & improve weapon and spell skills\\
\texttt{\#invoke} & use an artifact's power\\
\texttt{\#monster} & use a monster power when polymorphed\\
\texttt{\#turn} & turn undead (Priest/Knight or spell)\\
\texttt{\#chat} & talk (shop price quotes)\\
\texttt{\#ride} & mount or dismount a steed\\
\texttt{\#annotate} & note something about this level\\
\texttt{v} & event journal (chronicle)\\
\texttt{\^{}X} & your attributes and intrinsics\\
\texttt{S} & save and quit (resume later)\\
\end{tabular}
\end{minipage}
\par\vspace{1.4em}
{\centering
Commands written with \texttt{\#} are typed out in full.\\
A number repeats a command: \texttt{22s} searches 22 times, stopping early if anything happens.\\
\texttt{Esc} cancels; \texttt{Ctrl+A} repeats your last command.\par}
\normalsize
\clearpage
...
\item \textbf{Are you blinded, confused, or stunned?} Apply (\texttt{a}) a unicorn horn to clear it. (p.~\pageref*{neutralizing-ailments})
\item \textbf{Are you turning to stone?} Eat a lizard corpse or any acidic corpse (an acid blob will do) before the timer runs out. (p.~\pageref*{petrification-stoning})
\end{itemize}
...
\item melee a floating eye. Its gaze freezes you helpless while everything else closes in. (p.~\pageref*{eyes-and-spheres-e})
\item touch or wield a cockatrice corpse barehanded. That is instant stone. (p.~\pageref*{cockatrices-c})
\item pray again right after praying. Your god is not a vending machine. (p.~\pageref*{prayer})
\item swing from on top of your own Elbereth. It defiles the ward. (p.~\pageref*{defiling-the-name-of-elbereth-important})
\item eat a fresh corpse to ``heal.'' Corpses feed you, they don't heal you, and the wrong one can poison you. (p.~\pageref*{dangerous-foods})
\end{itemize}
...
\vspace*{2in}
\begin{center}
{\huge\bfseries NetHack Spoilers\par}
\vspace{0.8em}
{\Large\itshape A Traveler's Companion to the\\[0.2em]Mazes of Menace}
\end{center}
\end{titlepage}
...
\vspace*{\fill}
\begin{center}
{\itshape You feel like someone is helping you.}
\end{center}
\vspace*{\fill}
\clearpage
