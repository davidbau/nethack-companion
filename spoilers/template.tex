...
\vspace{0.9em}
\textbf{Dungeon actions}\ \ {\itshape(type the \texttt{\#} and the word)}\par\vspace{0.2em}
\begin{tabular}{@{}ll@{}}
...
\end{minipage}
\par\vspace{1.4em}
{\centering Most actions then ask for a direction or an item. \texttt{Esc} cancels; \texttt{Ctrl+A} repeats your last command.\par}
\normalsize
\clearpage
% --- Last-resort panic card (verso, faces the command reference) ---
\thispagestyle{empty}
{\centering{\large\bfseries When You're About to Die}\par\vspace{0.2em}
{\itshape One turn from death? Reach for these, roughly in this order.}\par}
\vspace{1.4em}
\footnotesize
\noindent\hfill\begin{minipage}{0.82\textwidth}
\textbf{First, pray} (p.~\pageref*{prayer}).\ \ \texttt{\#pray} asks your god
for rescue, and a god in good standing almost always answers, saving you from
critically low HP, starvation, turning to stone, turning to slime, or illness.
The catch: it works only if you haven't prayed in the last
\textasciitilde1{,}000 turns, your Luck isn't negative, and you aren't in
Gehennom. When in doubt, this is the button.

\vspace{1em}
\textbf{Or get out of the losing fight.}
\begin{itemize}[leftmargin=1.4em,itemsep=0.35em,topsep=0.3em]
\item \textbf{Drink your healing.} \texttt{q} a potion of full or extra healing. You are probably carrying one. This is the moment. (p.~\pageref*{potion-healing})
\item \textbf{Kill it outright.} Zap (\texttt{z}) a wand of death or wand of sleep at whatever is killing you. (p.~\pageref*{wand-death})
\item \textbf{Write Elbereth.} \texttt{E}, then type \texttt{Elbereth}, and stand on it. Most monsters lose their nerve and won't attack. (p.~\pageref*{elbereth})
\item \textbf{Teleport away.} Read a scroll of teleportation, or zap (\texttt{z}) a wand of teleportation at yourself. (p.~\pageref*{wand-teleportation})
\item \textbf{Drop through the floor.} Apply (\texttt{a}) a pick-axe or zap a wand of digging straight down, and leave the level behind. (p.~\pageref*{wand-digging})
\item \textbf{Run for the stairs.} \texttt{<} or \texttt{>} takes you off the level entirely. (p.~\pageref*{when-the-fight-goes-badly})
\item \textbf{Break up the mob.} A ring of conflict or a scroll of scare monster buys you room. (p.~\pageref*{scroll-scare-monster})
\item \textbf{Clear a deadly status.} Apply (\texttt{a}) a unicorn horn to shake off blindness, confusion, or stun when one of them is what's getting you killed. (p.~\pageref*{unicorn-horn-interactions})
\end{itemize}

\vspace{0.9em}
{\itshape An amulet of life saving revives you once, but only if you are already wearing it (p.~\pageref*{special-and-utility}). The surest escape is the one you set up before you needed it.}
\end{minipage}\hfill
\normalsize
\clearpage
