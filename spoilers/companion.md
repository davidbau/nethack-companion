...
The arrows `→`, `←`, `↑`, `↓` mark **rolling-boulder traps**. They
look like pits, but a boulder pushed onto one doesn't stop there:
it keeps rolling in the direction you pushed, traveling past any
already-filled holes and falling into the next unfilled hole it
hits, or against the first wall. The arrow on the map points
toward whichever side has the most unfilled pits along the trap's
line — that's the direction you want to push a boulder from to
make the trap fill a remote hole for free. The trap is permanent:
it doesn't go away after one boulder rolls over it, so it can be
used to fill several distant pits across multiple pushes.
